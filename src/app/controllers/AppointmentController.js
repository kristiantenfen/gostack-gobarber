...
class AppointmentController {
  async index(req, res) {
    ...
    return res.json({ data, total, page, limit, count: data.length });
  }
  ...
  async delete(req, res) {
    ...
    if (req.userId !== appointment.user_id) {
      return res
        .status(401)
        .json({ error: "You don't have permission to cancel this appoinment" });
    }
    ...
    if (isBefore(dateWithSub, new Date())) {
      return res.status(401).json({
        error:
          "You don't have permission to cancel an appoinment with less two hours",
      });
    }
